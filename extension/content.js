...
function injectPageBridge() {
  if (document.documentElement.dataset.sqvBridgeInjected === "true") {
    return;
  }

  const script = document.createElement("script");
  script.src = chrome.runtime.getURL("page-bridge.js");
  script.dataset.source = PAGE_SOURCE;
  script.async = false;
  script.onload = () => script.remove();
  (document.head || document.documentElement).appendChild(script);
  document.documentElement.dataset.sqvBridgeInjected = "true";
}
...
async function sendPageStatus() {
  ...
  const sqlSnapshot = pickBestSnapshot([bridgeSnapshot, collectDomSqlSnapshot()]);
...
function detectSupersetSqlLab() {
  ...
  const hasSqlEditor =
    SQL_EDITOR_SELECTORS.some((selector) => document.querySelector(selector)) ||
